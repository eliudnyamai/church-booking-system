...
<html lang="en">
  ...
  <body>
    <?php
     ...
     $table="speaker";
     $column="*";
     $data=$crud->selectData($table, $column);
    
     $table="admin";
     $column="*";
     $current_admins=$crud->selectData($table,$column);

     $table="session";
     $column="*";
     $sessions=$crud->selectData($table,$column);
    
    ?>
    <div class="sidebar">
        <a class="active"  href="#home">Dashboard</a>
        <a class="tablinks" onclick="displayForm(event, 'create-admin')" href="#news">Create Admin</a>
        <a class="tablinks" onclick="displayForm(event, 'add-speaker')" href="#contact">Add speaker</a>
        <a class="tablinks" onclick="displayForm(event, 'add-session')" href="#">Add session</a>
    </div>
    <div class="content  ">
    <!-- New Admin -->
      <Section id="create-admin" class="tabcontent row">
    ...
    <!-- Start of speaker form section -->
    <section id="add-speaker" class="tabcontent row">
        <div class="col-md-6 shadow">
   ...
   <!-- Start of session form section -->
  <section id="add-session" class="tabcontent row">
      <div class="col-md-6 shadow">
...
<!-- End of sesssion form session -->
<!-- Start of booking section -->
<section class="container-fluid row mx-auto " >
       <?php foreach($sessions as $key => $value): 
        $date=$value['Date'];
        ?>       
       <div id="bookings" class="col-md-6 shadow " style="overflow-x:auto" >
       <table class="table table-striped">
      ...
      <tbody>
          ...
          $i=0;
          foreach($crud->selectData('booking','*', $condition) as $key => $value):
          $i++;
          ?>
            <tr>
            <td><?=$i;?></td>
    ...
    <form class="generate-pdf" method="post" action="actions.php">
      <button  name="generate-pdf"  class="gradient btn text-white " >Generate PDF</button>
      <div class="form-group">
          <input  name="session_id" class="form-control" type="hidden" value="<?=$value['session_id']?>" readonly>
       </div>
    </form>  
  </div>  
<?php endforeach; ?>
